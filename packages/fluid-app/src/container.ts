...
export class InsecureRouterliciousTokenProvider implements ITokenProvider {
    ...
    public async fetchOrdererToken(tenantId: string, documentId: string, refresh: boolean = false): Promise<ITokenResponse> {
        return this.fetchToken("orderer", tenantId, documentId, refresh);
    }

    public async fetchStorageToken(tenantId: string, documentId: string, refresh: boolean = false): Promise<ITokenResponse> {
        return this.fetchToken("storage", tenantId, documentId, refresh);
    }

    private fetchToken(type: "orderer" | "storage", tenantId: string, documentId: string, refresh: boolean): ITokenResponse {
...
export class InsecureRouterliciousUrlResolver implements IUrlResolver {
    ...
    public async resolve(request: IRequest): Promise<IResolvedUrl> {
        ...
        const fluidUrl = `fluid://${this.ordererUrl.replace("http://", "")}/${encodeURIComponent(this.tenantId)}/${encodeURIComponent(this.documentId)}${reqUrl.search ?? ""}`;

        const deltaStorageUrl =
            `${this.ordererUrl}/deltas/${encodeURIComponent(this.tenantId)}/${encodeURIComponent(this.documentId)}`;
...
export async function getR11sContainer(
    ...
    const urlResolver = new InsecureRouterliciousUrlResolver(tokenProvider, tenantId, documentId, ordererUrl, storageUrl);
