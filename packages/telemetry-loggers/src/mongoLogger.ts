...
export class MongoLogger implements ILogger {
    ...
    private readonly logSubsetField: string;
    ...
    public async log(data: unknown): Promise<void> {
        ...
        const collection = await this.getCollection(subset);
    ...
    private async getCollection(subset: string): Promise<Collection<any>> {
    ...
    private getLogSubsetFromData(data: unknown): string {
        return (data as any)[this.logSubsetField] ?? "unknown";
    }
